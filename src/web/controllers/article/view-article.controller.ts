...
import { ViewArticleUseCase } from '@/application/usecases/article'

import { BaseError } from '@/application/errors'

import { ResponseHandler } from '@/web/utils'

export const viewArticleController = (repository: ArticleContract) => {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { id } = request.params as { id: string }

      const useCase = new ViewArticleUseCase(repository)
